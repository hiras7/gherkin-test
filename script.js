...
function renderOverview(list, totals){
  let tReq=0, tFit=0, tScn=0;
  if (totals && typeof totals==='object'){
    tReq = totals.totalRequirements ?? 0;
    tFit = totals.totalFitCriteria ?? 0;
    tScn = totals.totalScenarios ?? 0;
  } else {
    const arr = list || [];
    tReq = arr.length;
    tFit = arr.reduce((s,r)=>s+(r.FitCount??0),0);
    tScn = arr.reduce((s,r)=>s+(r.ScenarioCount??1),0);
  }
  const elReq=qs('#totReq'); const elFit=qs('#totFit'); const elScn=qs('#totScn');
  if(elReq) elReq.textContent=`Total number of requirements : ${tReq}`;
  if(elFit) elFit.textContent=`Total number of FIT criterion : ${tFit}`;
  if(elScn) elScn.textContent=`Total number of scenarios : ${tScn}`;
}
...
// Init
window.addEventListener('DOMContentLoaded', ()=>{ applyLogo(); initDragDrop(); enableButtons(); qs('#previewBtn')?.addEventListener('click', previewFile); qs('#generateBtn')?.addEventListener('click', generateFile); });
